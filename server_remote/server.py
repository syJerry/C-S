...
def get_summary(req:ChatRequest):
    res = llm_message(Prompt.SUMMARY_PROMPT,req.message,schema_type="summary")
    return ChatResponse(
        response=res
    )
...
def get_summary(req:ChatRequest):
    res = llm_message(Prompt.SYSTEM_PROMPT,req.message, schema_type="chat")
    print("="*60)
    print(res)
    print("="*60)
    res = llm_message(Prompt.PROCESS_PROMPT, res, schema_type="cited_answer")
    print(res)
    print("="*60)
    return ChatResponse(
        response=res
