...
# from model_util.Model import llm, embedding,llm_message
...
#     # print(docs_str)
#     response = llm_message(_JUDGE_SYSTEM , query, "relevance_judge")
#     raw = response
#     # 清理可能的 markdown 代码块
...
给定一个问题和一篇文档，请判断该文档是否直接用来回答该问题。

{human_message}

仅回答相关或者不相关，不要有其他内容。
"""
system = """提取下面内容的结论，如果结论为相关回答true，否则回答false：
...
def judge_relevance_with_llm(query: str, documents: List[Document],sim) -> List[bool]:
#     """逐条判断文档相关性，并汇总布尔结果。"""
    judgments: List[bool] = []
...
{content}
"""
        response = llm_message(_JUDGE_SYSTEM, user_message, "chat")

        judge = llm_message(system,response,"relevance_judge")
        print(judge)
        raw = repair_json(judge)
        parsed = json.loads(raw)
...
def run_single_test(
    ...
    # avg_similarity = sum(valid_sims) / len(valid_sims) if valid_sims else 0.0
    avg_similarity = (
        sum(score * weight for score, weight in zip(valid_sims, range(len(valid_sims), 0, -1)))
        / sum(range(len(valid_sims), 0, -1))
        if valid_sims else 0.0
    )
    if verbose:
    ...
    judgments = judge_relevance_with_llm(query, docs,sims)
    relevant_count = sum(judgments)
